...
split_seed = 3
name_final = '../checkpoints/beauty_3_roberta_final.pt'
name_best = '../checkpoints/beauty_3_roberta_best.pt'

df_cellphone = pd.read_csv('../dataset/beauty.csv.gz', compression='gzip')
...
df_subsample_train = training_data[['reviewText','helpfulness_ratio','asin']].sample(frac = 1, random_state = split_seed)
df_subsample_test = test_data[['reviewText','helpfulness_ratio','asin']].sample(frac = 1, random_state = split_seed)
df_subsample_val = val_data[['reviewText','helpfulness_ratio','asin']].sample(frac = 1, random_state = split_seed)
...
class RobertaRegressor(nn.Module):
    ...
    def __init__(self, drop_rate=0.2, freeze_Roberta=False, freeze_layers='all'):
    ...
    def forward(self, input_ids, attention_masks):
        ...
        #for average pooling replace above line with the below.
        #class_label_output = torch.mean(outputs[0] * torch.unsqueeze(attention_masks,dim=2),dim=1)
        outputs = self.regressor(class_label_output)
        return outputs
model = RobertaRegressor(drop_rate=0.0, freeze_Roberta=True, freeze_layers=10)
...
plt.savefig('../figs/roberta_beauty_3.png', dpi = 300)
